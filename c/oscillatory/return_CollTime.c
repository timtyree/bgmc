...
// ...ahh fuck, gcc wasn't compiling and I wasn't notified because bash just bashes through everything...
// ...oy vey, another few hours just to deduce I was using i instead of j in a for loop... fuck c programming... for now...
  ...
  */
  ...
  for (q = 0; q < niter; q++) {
    ...
    for (j = 0; j < Nmax; j++ ) {
      ...
      // //  Problemn Identified! random_phase computation causes the segmentation fault
      // // initialize a random_sign for their period for each particle
      // randbool = rand() & 1;
      // TODO: test whether setting float to double anywhere fixes the segmentation fault.
      // TODO: set random_phase[i] to 1. if uniformRandom is greater than 1/2. else, set it to -1.
      ...
      omega[j] = random_phase[j]*omega0;

      // // initialize a random_phase for each particle
      random_phase[j] = random_phase[j]*2.*3.141592653589793*uniformRandom();
      phase[j]=random_phase[j];
      // DONT(later): check if any particles are within the minimum allowable distance
      // DONT(later): reseed any particles that are within the minimum allowable distance
    }
    ...
    while(still_running){
      ...
      if (neighbor==1){
        ...
        for (j = 0; j < Nmax; j++ ) {
          ...
          // CONFIRMED: phase_difference must, because local changes to impact_factor have an impact, but multiplying by cos(phase_difference) does not
      ...
      for (j = 0; j < Nmax; j++ ) {
        ...
        for (i = 0; i < Nmax-1; i++ ) {
          for (j = i+1; j < Nmax; j++ ) {
            ...
            // this does not have an effect//
            // CONFIRMED: phase_difference must, because local changes to impact_factor have an impact, but multiplying by cos(phase_difference) does not
            // local test for effect
            // impulse_factor=impulse_factor-1.; // this has and effect//
            //yes, ^that effects Tavg in the Outputs...
